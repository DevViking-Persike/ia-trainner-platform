...
- **Banco e papéis.** Banco `ia_trainner`, schema `ia_trainner`. `ia_trainner_migrator` é dono do schema e executa DDL e dados de referência; `ia_trainner_app` é o papel de runtime da API e do Worker, com `SELECT/INSERT/UPDATE/DELETE` por privilégios padrão concedidos pelo migrador (`ALTER DEFAULT PRIVILEGES` no `V0001`) e sem `CREATE`, `ALTER`, `DROP`, `TRUNCATE` ou `TEMP`. O `infra-k8s` cria banco, papéis e schema; as credenciais ficam no Infisical: migrador em `/ia-trainner/sql-ddl`, aplicação em `/ia-trainner/backend`. Credenciais administrativas do servidor nunca são reaproveitadas.
