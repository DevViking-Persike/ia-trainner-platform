...
## URLs limpas, treinamento responsivo e renomeação — 26/09/2026

- [PR Angular #8](https://github.com/DevViking-Persike/ia-trainner-frontend-angular/pull/8), merge `71fddf01e84ba6e4b88d41fb7921c9ab5e33c958`: seta lateral acompanha recolher/expandir; campos de treinamento se adaptam à largura do painel; download de JSONL com três registros fictícios e explicação dos limites de ingestão (5.000 linhas / 32 MiB), sem alterar esses limites.
- Chat mostra `/app/chat`, detalhes mostram `/app/documentos/detalhe` e o filtro de coleção fica fora da URL. IDs continuam no estado interno e nas chamadas autenticadas. Recarga e Voltar/Avançar preservam a seleção na aba; um endereço limpo copiado para outra aba não seleciona aquele recurso. Links antigos continuam válidos. Trocas de documento cancelam leituras pendentes e descartam o conteúdo anterior.
- Login destaca o botão **Criar uma conta**, usando o cadastro já existente. A política vigente continua exigindo confirmação do e-mail e função `user` concedida administrativamente. A estrutura já permite múltiplos usuários: ZITADEL para identidade, PostgreSQL com `owner_sub` e MongoDB com `ownerSub`, com consultas filtradas pelo usuário autenticado. Nenhuma aprovação automática foi habilitada nesta entrega.
- Validação local: 264 testes e build aprovados; permanece o aviso preexistente de 7,98 kB no CSS do chat. Gitleaks sem achados nos commits e bundle. Prévia local conferida em desktop, painel estreito e celular; download do exemplo, envio sintético, recarga e Voltar/Avançar entre conversas com o mesmo endereço visível verificados no navegador.
- Executor renomeado para [ia-trainner-finetuning-py](https://github.com/DevViking-Persike/ia-trainner-finetuning-py), mantendo repository ID, histórico e visibilidade privada. Alias novo e antigo apontam para o mesmo checkout, sem duplicação. [PR infra-k8s #10](https://github.com/DevViking-Persike/infra-k8s/pull/10), merge `5d0419dcc12c7c4138574a1383d17ffeec07e6da`, atualiza Application, AppProject e URL da credencial gerenciada pelo Infisical. Dry-run no servidor e aplicação concluídos.
- [Actions Python 36256014539](https://github.com/DevViking-Persike/ia-trainner-finetuning-py/actions/runs/36256014539) aprovado na segunda tentativa: após a renomeação o GitHub ativou subject OIDC imutável; a confiança exata foi ajustada no Infisical sem ampliar audience, branch, ambiente, repository ID ou owner ID. A primeira tentativa recusou esse subject antes de publicar. Nenhum Job de GPU foi iniciado.
- Catálogo Kubernetes do executor verificado: revisão `prod-55219d5d39fc090edd9379401ab5bdd238d2a360-36256014539-2`, digest `sha256:7140ae69d108f1ee854fbf9489d9962a888dbfac803cc815b9c2f42b2750e415`, Argo `Synced/Healthy`, operação `Succeeded`. A publicação Angular desta entrega pode ser acompanhada no [Actions 36256443372](https://github.com/DevViking-Persike/ia-trainner-frontend-angular/actions/runs/36256443372).

## Correção do transporte de e-mail de contas — 26/09/2026

- Falha real observada no envio de orientação para conta existente: `SslHandshakeException`. O OpenSSL validou a cadeia/hostname no pod, mas MailKit 4.18.0 reproduziu `RevocationStatusUnknown` e `OfflineRevocation`, pois os serviços OCSP/CRL não estavam liberados na política de saída.
- [PR infra-k8s #11](https://github.com/DevViking-Persike/infra-k8s/pull/11), merge `177bab8`: política Cilium restrita ao backend permite DNS observado e HTTP/HTTPS apenas nos hosts Sectigo/USERTrust da cadeia SMTP. Dry-run aceito; manifesto de `main` aplicado no cluster. Nenhuma credencial, senha de usuário ou regra de aprovação foi alterada.
- Verificação no mesmo pod após a correção: MailKit conectou com `TLS policy: None`, verificação de revogação ativa e autenticação SMTP bem-sucedida. O diagnóstico não enviou mensagens e foi removido do pod. A entrega efetiva do e-mail e a situação da conta dependem do reenvio para o endereço do titular; este teste de conexão não comprova recebimento na caixa de entrada.
